...
pub async fn handler(
    ...
    let extension_activated = get_extension_activated(&mut transaction, &user.id).await?;
    let extension_activated_sent = get_extension_activated_sent(&mut transaction, &user.id).await?;
    let signature =
    ...
    if signature.verify(pubkey.as_ref(), message) {
        match db_user.wallet_address {
            None => {
                ...
                state
                    .wallet_addresses
                    .insert(user.email.clone(), Some(body.pubkey.clone()), None)
                    .await;
            }
            Some(wallet_address) => {
