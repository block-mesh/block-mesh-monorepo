...
use block_mesh_common::interfaces::server_api::IntractIdentityType;
...
static RATE_LIMIT: OnceCell<HashMapWithExpiry<String, ()>> = OnceCell::const_new();
...
pub async fn handler(
    ...
    if let Some(perk) = user_perks.iter().find(|i| i.name == PerkName::Intract) {
    ...
    if app_environment != "local" {
        ...
        cache.insert(email.to_string(), (), Some(date)).await;
    }
