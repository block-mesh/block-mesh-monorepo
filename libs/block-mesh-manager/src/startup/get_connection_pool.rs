            ...
            .before_acquire(|conn, _meta| {
                Box::pin(async move {
                    match sqlx::query_scalar::<_, Option<i64>>(
                        "SELECT txid_current_if_assigned()::bigint",
                    )
                    .fetch_one(&mut *conn)
                    .await
                    {
                        Ok(Some(_)) => {
                            tracing::warn!(
                                "before_acquire(get_connection_pool): rolling back stale write transaction"
                            );
                            if let Err(e) = sqlx::query("ROLLBACK").execute(&mut *conn).await {
                                tracing::warn!(
                                    "before_acquire(get_connection_pool): ROLLBACK failed, discarding connection: {e}"
                                );
                                return Ok(false);
                            }
                            Ok(true)
                        ...
                        Err(e) => {
                            tracing::warn!(
                                "before_acquire(get_connection_pool): health check failed, discarding connection: {e}"
                            );
                            Ok(false)
                        }
                    }
