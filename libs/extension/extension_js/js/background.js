...
async function get_polling_interval() {
  try {
    const response = await fetch(
      "https://feature-flags.blockmesh.xyz/read-flag/polling_interval"
    );
    if (response.ok) {
...
async function init_background() {
  ...
  setTimeout(() => {
    ...
    console.log("done starting websocket");
  }, 1000);
  await create_alarm().then(onSuccess, onError);
  await chrome.alarms.create("stayAlive", {
...
chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
  console.log(
    `Popup message received: ${JSON.stringify(request)}, ${JSON.stringify(sender)}`
  );
  return true;
